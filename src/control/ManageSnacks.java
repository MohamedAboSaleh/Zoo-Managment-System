...
public class ManageSnacks implements Initializable {
	...
	void addSSnackSelected(MouseEvent event) {
		...
		AddSnackControler addS = new AddSnackControler();
		try {
			addS.start(stage);// go to the add snack page
		} catch (Exception e) {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		firstTrait.setText("sprinkel");
		secondTrait.setText("straw");
		thirdTrait.setText("iceCube");
		firstTrait.setCellValueFactory(new PropertyValueFactory<>("sprinkel"));
		secondTrait.setCellValueFactory(new PropertyValueFactory<>("straw"));
		thirdTrait.setCellValueFactory(new PropertyValueFactory<>("iceCube"));
		...
		for (Snack s : allSnacks) {// get all the drinks
			if ((s instanceof Drink) == true)
				finalAllSnacks.add(s);
		}
		snacks.setItems(FXCollections.observableArrayList(finalAllSnacks));// add the drinks

		minPriceField.textProperty().addListener((observable, oldValue, newValue) -> {// this listener will change the
																						// drinks in the table view, the
																						// table view will contain only
																						// the drinks with price like
	...
	public void start(Stage stage) {
		...
		try {
			// load the ManageSnacks fxml file
			Parent root = FXMLLoader.load(getClass().getResource("/view/ManageSnacks.fxml"));
