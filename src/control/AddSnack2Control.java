...
public class AddSnack2Control implements Initializable {
	...
	void addSnackSelected(ActionEvent event) {
		...
		mediaPlayer.play();
		// check if one of the field still empty
		if (sFattening.getValue() == null || firstTrait.getValue() == null || sType.getValue() == null
			...
			try {
				...
				// define a new instance of food
				Snack s = new Food(sType.getValue(), sName.getText(), sBar.getValue(), sFattening.getValue(),
				...
				if (Zoo.getInstance().addSnack(s)) {// if the add method return true
					Alert a = new Alert(AlertType.CONFIRMATION, "Food added!", ButtonType.OK);
					a.showAndWait();
					Zoo.getInstance().saveToSerFile();// save the data to the .ser file
					Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
					ManageSnacks2Control manage = new ManageSnacks2Control();
					try {
						manage.start(stage);// return to the previous page
					} catch (Exception e) {
						Alert a1 = new Alert(AlertType.ERROR, e.getMessage(), ButtonType.OK);
						a1.showAndWait();
					}
				} else {
					Alert a = new Alert(AlertType.ERROR, "Couldn't add Food!", ButtonType.OK);
					a.showAndWait();
				}
	...
	void cancel(ActionEvent event) {
		...
		ManageSnacks2Control manage = new ManageSnacks2Control();
		try {
			manage.start(stage);// return to previous page
		} catch (Exception e) {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		sBar.getItems().addAll(Zoo.getInstance().getBars().values());// add all the bars to the combo box
		// set the text in the labels
		firstTraitLabel.setText("plate");
		secondTraitLabel.setText("spaciy");
		thirdTraitLabel.setText("glutenFree");
		addSnackLabel.setText("Add Food");
		sType.getItems().addAll(SnackType.Dairy_Snack, SnackType.Gluten_Free, SnackType.Meat, SnackType.Snack,
				SnackType.Vegiterian);// add the snack types to the combo box
	...
	public void start(Stage stage) {
		...
		try {
			// load the AddSnack2 fxml file
			Parent root = FXMLLoader.load(getClass().getResource("/view/AddSnack2.fxml"));
